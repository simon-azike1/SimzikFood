...
const TABLE_BG    = 'https://fact-magazine.com/wp-content/uploads/2023/03/DSC_9381.jpg'

const fadeUp  = { hidden: { opacity: 0, y: 35 }, show: { opacity: 1, y: 0, transition: { duration: 0.65, ease: [0.25, 0.1, 0.25, 1] } } }
const stagger = { show: { transition: { staggerChildren: 0.09 } } }

const SERVICES = [
  { icon: <Truck size={22} />,           title: 'Home Delivery',    desc: 'Freshly cooked West African meals delivered straight to your door across Rabat and Casablanca. Order 24h ahead and we handle the rest.',             highlight: 'Rabat & Casablanca'    },
  { icon: <Users size={22} />,           title: 'Family Packs',     desc: 'Feeding the whole family? Our large-format containers come with 4–6 generous portions — ideal for gatherings, celebrations, or meal prepping.',      highlight: 'Feeds 4–6 people'      },
  { icon: <UtensilsCrossed size={22} />, title: 'Single Meals',     desc: 'Individually portioned ~1L containers for a satisfying solo meal. Authentic flavors, generous portions, no effort required.',                          highlight: '~1L per container'     },
  { icon: <CalendarDays size={22} />,    title: 'Event Catering',   desc: 'Planning a party, gathering or office event? Get in touch to discuss bulk orders and custom catering arrangements for your occasion.',                  highlight: 'Contact us to arrange' },
  { icon: <Clock size={22} />,           title: 'Pre-Order Service',desc: 'We require 24 hours notice so every meal is freshly prepared just for you — never reheated, always made to order.',                                   highlight: '24h advance required'  },
  { icon: <MessageCircle size={22} />,   title: 'WhatsApp Ordering',desc: 'The easiest way to place your order. Send us a message with your order, delivery address, and preferred time.',                                        highlight: '+212 751 780853'       },
]

const PRICING = [
  { dish: 'Jollof Rice',               single: '60', family: '120' },
  { dish: 'Chicken Stew',              single: '70', family: '130' },
  { dish: 'Okro Soup',                 single: '65', family: '110' },
  { dish: 'Ogbono Soup',               single: '70', family: '120' },
  { dish: 'Vegetable Soup (Efo Riro)', single: '70', family: '120' },
  { dish: 'Fried Rice',                single: '60', family: '120' },
  { dish: 'Pepper Soup',               single: '65', family: '110' },
  { dish: 'Suya',                      single: '65', family: '110' },
  { dish: 'Palava Sauce',              single: '65', family: '110' },
  { dish: 'Liberian Jollof Rice',      single: '60', family: '120' },
  { dish: 'Pepper Butter Fish',        single: '70', family: '120' },
]
    ...
    <div className="min-h-screen bg-[#080808] text-white overflow-x-hidden">
          ...
          <motion.div initial="hidden" whileInView="show" viewport={{ once: true }} variants={stagger}
            ...
            {SERVICES.map(({ icon, title, desc, highlight }) => (
              <motion.div key={title} variants={fadeUp}
                ...
                className="group bg-[#0F0F0F] rounded-3xl p-7 transition-colors duration-300">
                ...
                <div className="inline-flex items-center gap-2 bg-[#40916C]/8 text-[#40916C] text-[11px] font-semibold px-3 py-1.5 rounded-full">
                  <CheckCircle size={11} />
                  {highlight}
                </div>
              </motion.div>
      ...
      <section className="py-20 md:py-28">
        <div className="max-w-3xl mx-auto px-6 md:px-10">
          <motion.div initial="hidden" whileInView="show" viewport={{ once: true }} variants={stagger}>
            <motion.div variants={fadeUp} className="bg-[#0F0F0F] rounded-3xl overflow-hidden">
              ...
              {PRICING.map(({ dish, single, family }, i) => (
                <motion.div key={dish} variants={fadeUp}
                  className={`grid grid-cols-3 px-6 md:px-8 py-4 items-center hover:bg-white/[0.02] transition-colors ${i < PRICING.length - 1 ? 'border-b border-white/[0.04]' : ''}`}>
              ...
              {[
                ...
                <motion.div key={n} variants={fadeUp} className="flex flex-col items-center">
                  <div className="relative w-16 h-16 rounded-2xl bg-[#0F0F0F] flex items-center justify-center text-[#F5C518] mb-5">
                    {icon}
                    <div className="absolute -top-2.5 -right-2.5 w-7 h-7 rounded-full bg-[#F5C518] text-[#080808] text-[10px] font-black flex items-center justify-center">{n}</div>
                  </div>
